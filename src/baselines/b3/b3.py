...
class PersonLevelModel(nn.Module):
    ...
    def _checkpoint(self, epoch,epochs, train_loss, val_loss, output_path):
        if epochs - epoch <= 5:
            checkpoint = {
                "epoch": epoch + 1,
    ...
    def forward(self, trainLoader, valLoader, epochs, output_path, device):
        ...
        for epoch in range(epochs):
            ...
            self._checkpoint(epoch, epochs, train_loss, val_loss, output_path)
        ...
        # Download file from kaggle
        # display(FileLink(backbone_filename.replace("/kaggle/working/", ""),
        #                  result_html_prefix="click here to download backbone model state dict: "))
        # display(FileLink(classifier_filename.replace("/kaggle/working/", ""),
        #                  result_html_prefix="click here to download classifier model state dict: "))
